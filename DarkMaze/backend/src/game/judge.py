...
def _parse_map(map_string, map_size, reversal_nodes=[]):
    ...
    for binary in binary_strings:
        ...
        binary_grid.extend([first_half % 2, second_half % 2])

    while len(binary_grid) < width * height:
        binary_grid.append(0)

    binary_grid = binary_grid[: width * height]

    swiper = np.array(binary_grid).reshape((height, width))
...
def _load_maze_from_json(maze_level_name):
    with open(
        "./src/game/maze_level/" + maze_level_name + ".json", "r", encoding="utf-8"
    ) as f:
        data = json.load(f)
